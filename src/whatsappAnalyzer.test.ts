...
describe('OCOD5 WhatsApp analysis',()=>{
 ...
 it('recognizes inbound replies and retains full routing numbers',()=>{expect(inbound).toMatchObject({status:'Inbound message received',direction:'Inbound',contextualReplyId:'msg-100'});expect(inbound.maskedCustomer).toBe('6598175528');expect(inbound.maskedBusiness).toBe('60111111111');expect(inbound.events[0].rawLine).toContain('customer reply')})
