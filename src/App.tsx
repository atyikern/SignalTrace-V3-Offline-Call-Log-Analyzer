...
import type { AgentAnalysis, AnalysisResult, AsteriskIvrCall, ExtensionNetworkAnalysis, IvrCall, LogType, NetworkSeverity, VoiceCall, VoiceExtensionAnalysis } from './types'
...
function EmptyState({ logType, file, onLogType, onUpload, onAnalyze }: { logType: LogType | ''; file?: File; onLogType: (type: LogType | '') => void; onUpload: () => void; onAnalyze: () => void }) {
  return <main className="empty-main">
    <section className="hero-simple">
      ...
      <div className="format-note"><FileText size={16} /><span><strong>Supported Logs</strong><small>OpsCentral SocketIO / EFV<br />Asterisk / FreePBX<br />Asterisk-IVR<br />PJSIP RTT / Reachability<br />eFrontVoice Call Routing / Agent<br />.log / .txt · Up to 50 MB</small></span></div>
    </section>
...
function Results({ result, selectedAgent, selectedExtension, selectedPhoneNumber, selectedIvrCall, selectedVoiceCallerId, selectedVoiceCall, selectedVoiceExtension, selectedAsteriskIvrCall, onSelectAgent, onSelectExtension, onSelectPhoneNumber, onSelectIvrCall, onSelectVoiceCallerId, onSelectVoiceCall, onSelectVoiceExtension, onSelectAsteriskIvrCall, onReset }: { result: AnalysisResult; selectedAgent?: AgentAnalysis; selectedExtension?: ExtensionNetworkAnalysis; selectedPhoneNumber?: string; selectedIvrCall?: IvrCall; selectedVoiceCallerId?: string; selectedVoiceCall?: VoiceCall; selectedVoiceExtension?: VoiceExtensionAnalysis; selectedAsteriskIvrCall?: AsteriskIvrCall; onSelectAgent: (key: string) => void; onSelectExtension: (extension: string) => void; onSelectPhoneNumber: (phone: string) => void; onSelectIvrCall: (callId: string) => void; onSelectVoiceCallerId:(id:string)=>void; onSelectVoiceCall:(id:string)=>void; onSelectVoiceExtension:(id:string)=>void; onSelectAsteriskIvrCall:(id:string)=>void; onReset: () => void }) {
  ...
  return <main className="result-page">
    ...
    <div className="extension-results">{selectedAsteriskIvrCall && <AsteriskIvrReport call={selectedAsteriskIvrCall} calls={result.asteriskIvrCalls} />}{selectedVoiceCall && <VoiceCallReport call={selectedVoiceCall} />}{selectedVoiceExtension && <VoiceExtensionReport analysis={selectedVoiceExtension} />}{selectedIvrCall && <IvrReport call={selectedIvrCall} />}{selectedExtension && <ExtensionReport analysis={selectedExtension} />}</div>
  </main>
}
...
export default function App() {
  ...
  const clearAnalysis = () => {
    ...
    setSelectedVoiceCallerId(undefined); setSelectedVoiceCall(undefined); setSelectedVoiceExtension(undefined); setSelectedAsteriskIvrCall(undefined)
  }
  ...
  return <div className="app" onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
    <header className="app-header"><button className="brand" onClick={reset} aria-label="SignalTrace home"><span className="brand-mark"><i /><i /><i /></span><span><strong>SignalTrace <b>V7</b></strong><small>Log, Voice &amp; Network Analyzer</small></span></button><div className="offline-badge"><span /> Offline mode</div></header>
