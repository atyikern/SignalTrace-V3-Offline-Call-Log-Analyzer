...
const LOG_TYPE_GROUPS: Array<{ label:string; options:Array<{value:LogType;label:string}> }> = [
  { label:'Voice', options:[{value:'efrontvoice',label:'eFrontVoice'},{value:'efrontvoice-ivr',label:'eFrontVoice-IVR'},{value:'routing-delay',label:'Routing Delay Analysis'},{value:'asterisk-ivr',label:'Asterisk-IVR'},{value:'asterisk-voicemail',label:'Asterisk/PBX Voicemail Analysis'}] },
  ...
  return <main className="empty-main">
    <section className="hero-simple">
      ...
      <div className="upload-workflow">
        ...
        {logType==='efrontvoice-ivr'&&<details className="threshold-settings"><summary>V9 routing thresholds</summary>{Object.entries(thresholds).map(([key,value])=><label key={key}>{key.replaceAll(/([A-Z])/g,' $1')} (ms)<input type="number" min="0" value={value} onChange={event=>onThresholds({...thresholds,[key]:Number(event.target.value)})}/></label>)}</details>}{logType==='ocod5-whatsapp'&&<details className="threshold-settings"><summary>WhatsApp timing thresholds</summary>{Object.entries(whatsappThresholds).map(([key,value])=><label key={key}>{key.replaceAll(/([A-Z])/g,' $1')} (ms)<input type="number" min="0" value={value} onChange={event=>onWhatsappThresholds({...whatsappThresholds,[key]:Number(event.target.value)})}/></label>)}</details>}{(logType==='efrontvoice-ivr'||logType==='ocod5-whatsapp'||logType==='routing-delay'||logType==='messaging-routing-delay'||logType==='messaging-license')&&<label className="paste-log"><span className="upload-label">Or Paste Log</span><textarea aria-label="Paste Log" value={pasted} onChange={event=>onPaste(event.target.value)} placeholder="Paste log records here" /></label>}<button className="primary-button" disabled={!isSelectableLogType(logType) || (!files.length&&!pasted.trim())} onClick={onAnalyze}><Upload size={19} />Analyze</button>
      </div>
