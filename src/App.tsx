...
function Results({ result, selectedAgent, selectedExtension, onSelectAgent, onSelectExtension, onReset }: { result: AnalysisResult; selectedAgent?: AgentAnalysis; selectedExtension?: ExtensionNetworkAnalysis; onSelectAgent: (key: string) => void; onSelectExtension: (extension: string) => void; onReset: () => void }) {
  ...
  return <main className="result-page">
    ...
    <div className="extension-results">{selectedExtension && <ExtensionReport analysis={selectedExtension} />}</div>
  </main>
}
...
export default function App() {
  ...
  const reset = () => { setResult(undefined); setSelectedAgent(undefined); setSelectedExtension(undefined); setError(undefined) }
  ...
  return <div className="app" onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
    ...
    {result ? <Results result={result} selectedAgent={result.agents.find((agent) => agent.key === selectedAgent)} selectedExtension={result.extensions.find((extension) => extension.extension === selectedExtension)} onSelectAgent={setSelectedAgent} onSelectExtension={setSelectedExtension} onReset={reset} /> : <EmptyState onUpload={() => inputRef.current?.click()} />}
