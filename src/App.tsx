...
import type { AgentAnalysis, AnalysisResult, AsteriskIvrCall, ExtensionNetworkAnalysis, IvrCall, LogType, RoutingDelayAnalysis, VoiceCall, VoiceExtensionAnalysis, VoicemailCallAnalysis, WebhookTransaction, WhatsappMessageAnalysis } from './types'
import { SharedAnalysisReport } from './SharedAnalysisReport'
import { normalizeAgent, normalizeAsterisk, normalizeExtension, normalizeIvr, normalizeRoutingDelay, normalizeVoiceCall, normalizeVoiceExtension, normalizeVoicemail, normalizeWebhook, normalizeWhatsapp } from './normalizedResults'
...
function RoutingDelayReport({analysis}:{analysis:RoutingDelayAnalysis}) {
 return <SharedAnalysisReport result={normalizeRoutingDelay(analysis)} downloadFileName={`routing-delay-${analysis.customerNumber??'unknown'}.json`}/>
}

interface CaseSelection { agent?:AgentAnalysis;extension?:ExtensionNetworkAnalysis;ivr?:IvrCall;voiceCall?:VoiceCall;voiceExtension?:VoiceExtensionAnalysis;asterisk?:AsteriskIvrCall;voicemail?:VoicemailCallAnalysis;webhook?:WebhookTransaction;whatsapp?:WhatsappMessageAnalysis;routingDelay?:RoutingDelayAnalysis }
...
export default function App() {
  ...
  const clearAnalysis = () => {
    ...
    setSelectedVoiceCallerId(undefined); setSelectedVoiceCall(undefined); setSelectedVoiceExtension(undefined); setSelectedAsteriskIvrCall(undefined); setSelectedVoicemailCall(undefined); setSelectedWebhookTransaction(undefined); setSelectedWhatsappMessage(undefined); setSelectedRoutingDelay(undefined)
  }
  ...
  return <div className="app" onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
    <header className="app-header"><button className="brand" onClick={reset} aria-label="SignalTrace home"><span className="brand-mark"><i /><i /><i /></span><span><strong>SignalTrace <b>V13</b></strong><small>Voice, Messaging &amp; Connectivity Analyzer</small></span></button><div className="header-actions"><button className="history-button" onClick={()=>setView('history')}><History size={16}/>Case History</button><div className="offline-badge"><span /> Offline mode</div></div></header>
