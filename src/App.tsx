...
import type { AgentAnalysis, AnalysisResult, AsteriskIvrCall, ExtensionNetworkAnalysis, IvrCall, LogType, VoiceCall, VoiceExtensionAnalysis, VoicemailCallAnalysis, WebhookTransaction, WhatsappMessageAnalysis } from './types'
import { SharedAnalysisReport } from './SharedAnalysisReport'
import { normalizeAgent, normalizeAsterisk, normalizeExtension, normalizeIvr, normalizeVoiceCall, normalizeVoiceExtension, normalizeVoicemail, normalizeWebhook, normalizeWhatsapp } from './normalizedResults'
...
interface CaseSelection { agent?:AgentAnalysis;extension?:ExtensionNetworkAnalysis;ivr?:IvrCall;voiceCall?:VoiceCall;voiceExtension?:VoiceExtensionAnalysis;asterisk?:AsteriskIvrCall;voicemail?:VoicemailCallAnalysis;webhook?:WebhookTransaction;whatsapp?:WhatsappMessageAnalysis }
...
export default function App() {
  ...
  const clearAnalysis = () => {
    ...
    setSelectedVoiceCallerId(undefined); setSelectedVoiceCall(undefined); setSelectedVoiceExtension(undefined); setSelectedAsteriskIvrCall(undefined); setSelectedVoicemailCall(undefined); setSelectedWebhookTransaction(undefined); setSelectedWhatsappMessage(undefined)
  }
  ...
  const analyze = async () => {
    ...
    if (!uploadedFiles.length) { applyAnalysis(pastedLog, 'Pasted eFrontVoice-IVR log'); return }
  ...
  return <div className="app" onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
    <header className="app-header"><button className="brand" onClick={reset} aria-label="SignalTrace home"><span className="brand-mark"><i /><i /><i /></span><span><strong>SignalTrace <b>V12</b></strong><small>Voice, Messaging &amp; Connectivity Analyzer</small></span></button><div className="header-actions"><button className="history-button" onClick={()=>setView('history')}><History size={16}/>Case History</button><div className="offline-badge"><span /> Offline mode</div></div></header>
