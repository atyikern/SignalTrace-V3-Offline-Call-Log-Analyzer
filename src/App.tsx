...
function AgentRoutingSection({ analysis, transactions }: { analysis: AgentRoutingAnalysis; transactions: WebhookTransaction[] }) {
 const routings=transactions.map(item=>item.agentRouting).filter((item):item is AgentRoutingAnalysis=>Boolean(item));const found=routings.filter(item=>['Agent Found','Agent Found After Retry'].includes(item.finalStatus)).length;const retry=routings.filter(item=>item.finalStatus==='Agent Found After Retry').length;const none=routings.filter(item=>item.finalStatus==='No Available Agent').length;const incomplete=routings.filter(item=>item.finalStatus==='Incomplete Agent Routing').length;const searches=routings.reduce((sum,item)=>sum+item.lookupAttempts,0);const disconnects=routings.reduce((sum,item)=>sum+item.disconnectionCount,0);const bool=(value?:boolean)=>value===undefined?'—':String(value)
 const actions=analysis.finalStatus==='No Available Agent'?['Confirm agents are logged in and Ready.','Check the assigned agent group.','Review language and product skill requirements.','Check whether routing criteria are too restrictive.','Review queue retry and overflow configuration.']:analysis.disconnectionCount?['Check CallFront service availability.','Review network connectivity between Webhook and CallFront.','Check whether a successful reconnection occurred.','Review repeated disconnect/reconnect events.','Check whether the interruption delayed agent routing.']:[]
 return <section className="agent-routing-analysis"><div className="agent-routing-heading"><span className="section-label">Agent Routing Analysis</span><h2>CallFront available-agent lookup</h2></div><div className="metric-row agent-routing-counters"><div><span>Agent Searches</span><b>{searches}</b></div><div><span>Agents Found</span><b className="routing-found">{found}</b></div><div><span>Found After Retry</span><b className="routing-retry">{retry}</b></div><div><span>No Available Agent</span><b className="routing-none">{none}</b></div><div><span>Incomplete Lookups</span><b className="routing-incomplete">{incomplete}</b></div><div><span>CallFront Disconnections</span><b className="routing-none">{disconnects}</b></div></div><div className="metric-row agent-routing-details"><div><span>Customer Number</span><b>{analysis.customerNumber??'—'}</b></div><div><span>WebSocket Session</span><b>{analysis.webSocketSession??'—'}</b></div><div><span>RoutingEntry ID</span><b>{analysis.routingEntryId??'—'}</b></div><div><span>Client Start Time</span><b>{timeOnly(analysis.clientStartTime)}</b></div><div><span>Agent Search Time</span><b>{timeOnly(analysis.agentSearchTime)}</b></div><div><span>Agent Response Time</span><b>{timeOnly(analysis.agentResponseTime)}</b></div><div><span>Agent Lookup Delay</span><b>{analysis.agentLookupDelayMs===undefined?'—':analysis.agentLookupDelayMs<1000?`${analysis.agentLookupDelayMs} ms`:`${(analysis.agentLookupDelayMs/1000).toFixed(2)} sec`}</b></div><div><span>Selected Agent ID</span><b>{analysis.selectedAgentId??'—'}</b></div><div><span>Lookup Attempts</span><b>{analysis.lookupAttempts}</b></div><div><span>Final Routing Status</span><b>{analysis.finalStatus}</b></div><div><span>Connection Warning</span><b>{analysis.connectionWarning??'None'}</b></div><div><span>Processing Value</span><b>{analysis.processingValue??'—'}</b></div><div><span>Maximum Processing Value</span><b>{analysis.maximumProcessingValue??'—'}</b></div><div><span>Maximum Consumed</span><b>{analysis.usagePercentage===undefined?'—':`${analysis.usagePercentage.toFixed(2)}%`}</b></div><div><span>Preferred Language</span><b>{bool(analysis.preferredLanguage)}</b></div><div><span>Default Language</span><b>{bool(analysis.defaultLanguage)}</b></div><div><span>Preferred Product</span><b>{bool(analysis.preferredProduct)}</b></div><div><span>Default Product</span><b>{bool(analysis.defaultProduct)}</b></div></div><div className="agent-routing-timeline"><span className="section-label">Agent Routing Timeline</span>{analysis.events.map((event,index)=><div className="problem-time" key={`${event.timestampMs}:${event.type}:${index}`}><time>{timeOnly(event.timestamp)}</time><div><span className="indicator"><i />{event.label}</span></div></div>)}</div><div className="agent-routing-finding"><span className="section-label">Agent Routing Finding</span><p>{analysis.finding}</p>{actions.length>0&&<ul>{actions.map(action=><li key={action}>{action}</li>)}</ul>}</div></section>
}
