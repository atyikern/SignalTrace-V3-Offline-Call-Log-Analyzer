...
import type { AgentAnalysis, AnalysisResult, AsteriskIvrCall, CallQueueAnalysis, ExtensionNetworkAnalysis, IvrCall, LicenseOccupancyAnalysis, LogType, RoutingDelayAnalysis, VoiceCall, VoiceExtensionAnalysis, VoicemailCallAnalysis, WebhookTransaction, WhatsappMessageAnalysis } from './types'
import { SharedAnalysisReport } from './SharedAnalysisReport'
import { normalizeAgent, normalizeAsterisk, normalizeCallQueue, normalizeExtension, normalizeIvr, normalizeLicenseOccupancy, normalizeRoutingDelay, normalizeVoiceCall, normalizeVoiceExtension, normalizeVoicemail, normalizeWebhook, normalizeWhatsapp } from './normalizedResults'
...
const LOG_TYPE_GROUPS: Array<{ label:string; options:Array<{value:LogType;label:string}> }> = [
  { label:'Voice', options:[{value:'efrontvoice',label:'eFrontVoice'},{value:'efrontvoice-ivr',label:'eFrontVoice-IVR'},{value:'routing-delay',label:'Routing Delay Analysis'},{value:'call-queue',label:'Call Queue Analysis'},{value:'asterisk-ivr',label:'Asterisk-IVR'},{value:'asterisk-voicemail',label:'Asterisk/PBX Voicemail Analysis'}] },
  { label:'Messaging', options:[{value:'opscentral-webhook',label:'Webhook'},{value:'ocod5-whatsapp',label:'OCOD5 WhatsApp'},{value:'messaging-routing-delay',label:'Routing Delay Analysis'}] },
  ...
  return <main className="empty-main">
    <section className="hero-simple">
      ...
      <div className="upload-workflow">
        ...
        {logType==='efrontvoice-ivr'&&<details className="threshold-settings"><summary>V9 routing thresholds</summary>{Object.entries(thresholds).map(([key,value])=><label key={key}>{key.replaceAll(/([A-Z])/g,' $1')} (ms)<input type="number" min="0" value={value} onChange={event=>onThresholds({...thresholds,[key]:Number(event.target.value)})}/></label>)}</details>}{logType==='ocod5-whatsapp'&&<details className="threshold-settings"><summary>WhatsApp timing thresholds</summary>{Object.entries(whatsappThresholds).map(([key,value])=><label key={key}>{key.replaceAll(/([A-Z])/g,' $1')} (ms)<input type="number" min="0" value={value} onChange={event=>onWhatsappThresholds({...whatsappThresholds,[key]:Number(event.target.value)})}/></label>)}</details>}{(logType==='efrontvoice-ivr'||logType==='call-queue'||logType==='ocod5-whatsapp'||logType==='routing-delay'||logType==='messaging-routing-delay'||logType==='messaging-license')&&<label className="paste-log"><span className="upload-label">Or Paste Log</span><textarea aria-label="Paste Log" value={pasted} onChange={event=>onPaste(event.target.value)} placeholder="Paste log records here" /></label>}<button className="primary-button" disabled={!isSelectableLogType(logType) || (!files.length&&!pasted.trim())} onClick={onAnalyze}><Upload size={19} />Analyze</button>
      </div>
      <CheckingScriptsPanel logType={logType} />
      </div>
    </section>
    <section className="trust-strip"><div><LockKeyhole /><span><strong>Voice diagnostics</strong><small>Analyze IVR flow, routing delays, agent routing, and call failures.</small></span></div><div><ShieldCheck /><span><strong>Messaging diagnostics</strong><small>Trace WhatsApp delivery and webhook events.</small></span></div><div><FileLock2 /><span><strong>Connectivity diagnostics</strong><small>Identify ECONNRESET, RTT, and reachability issues.</small></span></div></section>
  </main>
}

interface SelectorOption { value: string; label: string }

function AnalysisEntitySelector({ label, value, options, onChange }: { label: string; value: string|VoiceCall; options: SelectorOption[]; onChange: (value: string) => void }) {
  return <label className="agent-selector"><span>{label}</span><div><select value={typeof value==='string'?value:voiceCallKey(value)} onChange={(event) => onChange(event.target.value)}>{options.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}</select><ChevronDown size={16} /></div></label>
}
...
function Results({ result, selectedAgent, selectedExtension, selectedPhoneNumber, selectedIvrCall, selectedVoiceCallerId, selectedVoiceCall, selectedVoiceExtension, selectedAsteriskIvrCall, selectedVoicemailCall, selectedWebhookTransaction, selectedWhatsappMessage, selectedRoutingDelay, selectedCallQueue, selectedLicenseOccupancy, onSelectAgent, onSelectExtension, onSelectPhoneNumber, onSelectIvrCall, onSelectVoiceCallerId, onSelectVoiceCall, onSelectVoiceExtension, onSelectAsteriskIvrCall, onSelectVoicemailCall, onSelectWebhookTransaction, onSelectWhatsappMessage, onSelectRoutingDelay, onSelectCallQueue, onSaveCase, onReset }: { result: AnalysisResult; selectedAgent?: AgentAnalysis; selectedExtension?: ExtensionNetworkAnalysis; selectedPhoneNumber?: string; selectedIvrCall?: IvrCall; selectedVoiceCallerId?: string; selectedVoiceCall?: VoiceCall; selectedVoiceExtension?: VoiceExtensionAnalysis; selectedAsteriskIvrCall?: AsteriskIvrCall; selectedVoicemailCall?:VoicemailCallAnalysis; selectedWebhookTransaction?: WebhookTransaction; selectedWhatsappMessage?:WhatsappMessageAnalysis; selectedRoutingDelay?:RoutingDelayAnalysis; selectedCallQueue?:CallQueueAnalysis; selectedLicenseOccupancy?:LicenseOccupancyAnalysis; onSelectAgent: (key: string) => void; onSelectExtension: (extension: string) => void; onSelectPhoneNumber: (phone: string) => void; onSelectIvrCall: (callId: string) => void; onSelectVoiceCallerId:(id:string)=>void; onSelectVoiceCall:(id:string)=>void; onSelectVoiceExtension:(id:string)=>void; onSelectAsteriskIvrCall:(id:string)=>void; onSelectVoicemailCall:(id:string)=>void; onSelectWebhookTransaction:(id:string)=>void; onSelectWhatsappMessage:(id:string)=>void; onSelectRoutingDelay:(id:string)=>void; onSelectCallQueue:(id:string)=>void; onSaveCase:()=>void; onReset: () => void }) {
  if (!result.agents.length && !result.extensions.length && !result.ivrCalls.length && !result.voiceCalls.length && !result.voiceExtensions.length && !result.asteriskIvrCalls.length && !result.voicemailCalls.length && !result.webhookTransactions.length && !result.whatsappMessages.length && !result.routingDelayAnalyses.length && !result.callQueueAnalyses.length && !result.licenseOccupancyAnalyses.length) return <main className="no-results"><AlertTriangle size={38} /><h2>No supported analysis found</h2><p>No supported voice, messaging, or connectivity events were detected for the selected log type.</p><button className="secondary-button" onClick={onReset}>Choose another log</button></main>
  return <main className="result-page">
    <div className="result-toolbar"><div className="file-summary"><FileText size={18}/><span><strong>{result.fileName}</strong><small>Log Type: {LOG_TYPES.find(type=>type.value===result.logType)?.label} · {result.totalLines.toLocaleString()} records · {result.callQueueAnalyses.length} Call Queue Analyses</small></span></div><div className="entity-selectors">{selectedCallQueue&&<AnalysisEntitySelector label="Selected Queue Call" value={selectedCallQueue.key} options={result.callQueueAnalyses.map(item=>({value:item.key,label:`${item.callerId??'Unknown caller'} · ${item.transactionId} · ${item.classification}`}))} onChange={onSelectCallQueue}/>} {selectedRoutingDelay&&<AnalysisEntitySelector label="Selected Routing Delay" value={selectedRoutingDelay.webSocketSession??selectedRoutingDelay.customerNumber??'routing-delay'} options={result.routingDelayAnalyses.map((item,index)=>({value:item.webSocketSession??item.customerNumber??`routing-delay-${index}`,label:item.customerNumber??'Unknown'}))} onChange={onSelectRoutingDelay}/>} {selectedWhatsappMessage&&<AnalysisEntitySelector label="Selected WhatsApp Message" value={selectedWhatsappMessage.key} options={result.whatsappMessages.map(item=>({value:item.key,label:item.status}))} onChange={onSelectWhatsappMessage}/>} {selectedVoicemailCall&&<AnalysisEntitySelector label="Selected Voicemail Call" value={selectedVoicemailCall.key} options={result.voicemailCalls.map(item=>({value:item.key,label:item.callId??'Unknown call'}))} onChange={onSelectVoicemailCall}/>} {selectedAsteriskIvrCall&&<AnalysisEntitySelector label="Selected Call" value={selectedAsteriskIvrCall.key} options={result.asteriskIvrCalls.map(item=>({value:item.key,label:item.key}))} onChange={onSelectAsteriskIvrCall}/>} {selectedVoiceCallerId&&<AnalysisEntitySelector label="Selected Voice Caller ID" value={selectedVoiceCallerId} options={[...new Set(result.voiceCalls.map(item=>item.callerId))].map(value=>({value,label:value}))} onChange={onSelectVoiceCallerId}/>} {selectedVoiceCall&&<AnalysisEntitySelector label={result.voiceCalls.some(item=>item.transactionId)?'Selected Transaction':'Selected Voice Call'} value={selectedVoiceCall} options={result.voiceCalls.filter(item=>item.callerId===selectedVoiceCallerId).map(item=>({value:voiceCallKey(item),label:item.transactionId?`TID ${item.transactionId}`:item.callId}))} onChange={onSelectVoiceCall}/>} {selectedVoiceExtension&&<AnalysisEntitySelector label="Selected Voice Extension" value={selectedVoiceExtension.extension} options={result.voiceExtensions.map(item=>({value:item.extension,label:item.extension}))} onChange={onSelectVoiceExtension}/>} {selectedPhoneNumber&&<AnalysisEntitySelector label="Selected Phone Number" value={selectedPhoneNumber} options={[...new Set(result.ivrCalls.map(item=>item.phoneNumber))].map(value=>({value,label:value}))} onChange={onSelectPhoneNumber}/>} {selectedIvrCall&&<AnalysisEntitySelector label="Selected Call" value={selectedIvrCall.callId} options={result.ivrCalls.filter(item=>item.phoneNumber===selectedPhoneNumber).map(item=>({value:item.callId,label:item.callId}))} onChange={onSelectIvrCall}/>} {selectedAgent&&<AnalysisEntitySelector label="Selected Agent" value={selectedAgent.key} options={result.agents.map(item=>({value:item.key,label:item.agent}))} onChange={onSelectAgent}/>} {selectedExtension&&<AnalysisEntitySelector label="Selected Extension" value={selectedExtension.extension} options={result.extensions.map(item=>({value:item.extension,label:item.extension}))} onChange={onSelectExtension}/>}</div><button className="new-log save-case-button" onClick={onSaveCase}><Save size={15}/>Save Case</button><button className="new-log" onClick={onReset}><Upload size={15}/>New log</button></div>
    <h1 className="analysis-title">Analysis Results</h1>
    {selectedAgent && <SharedAnalysisReport result={normalizeAgent(selectedAgent)} />}
    <div className="extension-results">{selectedCallQueue&&<CallQueueReport analysis={selectedCallQueue}/>} {selectedLicenseOccupancy&&<LicenseOccupancyReport analysis={selectedLicenseOccupancy}/>} {selectedRoutingDelay&&<RoutingDelayReport analysis={selectedRoutingDelay}/>} {selectedVoicemailCall&&<VoicemailReport call={selectedVoicemailCall}/>} {selectedWhatsappMessage&&<WhatsappReport message={selectedWhatsappMessage}/>} {selectedWebhookTransaction && <WebhookReport transaction={selectedWebhookTransaction} transactions={result.webhookTransactions} onSelect={onSelectWebhookTransaction} />}{selectedAsteriskIvrCall && <AsteriskIvrReport call={selectedAsteriskIvrCall} calls={result.asteriskIvrCalls} />}{selectedVoiceCall && <VoiceCallReport call={selectedVoiceCall} />}{selectedVoiceExtension && <VoiceExtensionReport analysis={selectedVoiceExtension} />}{selectedIvrCall && <IvrReport call={selectedIvrCall} />}{selectedExtension && <ExtensionReport analysis={selectedExtension} />}</div>
  </main>
}
...
export default function App() {
  ...
  const [selectedCallQueue,setSelectedCallQueue]=useState<string>()
  ...
  const analyze = async () => {
    ...
    const applyAnalysis = (contents:string,name:string) => {
      ...
      setSelectedCallQueue(analysis.callQueueAnalyses[0]?.key)
